...
export default function CPQAdvanced() {
  ...
  const updateComponent = (
    id: string,
    field: 'name' | 'unitLaborCost' | 'laborHours' | 'unitMaterialCost' | 'quantity' | 'Icon',
        ...
        <header className="mb-6 flex flex-col items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-gray-300 text-xs sm:text-sm">
              ...
              stream—auto‐configuring live quotes, dynamic pricing, and profit analysis.
            </p>
            <details className="mt-2 text-gray-400 text-xs sm:text-sm [&_summary]:cursor-pointer [&_summary]:text-[#05c8fb]">
              ...
              <div className="mt-2 space-y-2">
                <p>
                  CPQ = Configure–Price–Quote. Each software asset (code, PDF, image, etc.) is a
                  monetizable component. Freelancers can bundle and sell historical code/work with
                  real-time profitability controls.
                </p>
                <p>
                  Manager can: rename a component, pick an icon, set labor &amp; material cost, and
                  apply global margin/markup/discount. The system then calculates a customer‐facing
                  unit price and profit.
                </p>
                <p>
                  Customer can only choose which components and quantities, then see final price and
                  request a quote.
                </p>
              </div>
            </details>
                ...
                <div className="flex-[2] flex items-center gap-1 sm:gap-2 px-1 sm:px-2">
                  {isManagerView ? (
                    <>
                      ...
                      />
                      <Listbox
                        value={iconOptions.find((opt) => (opt.Icon === comp.Icon ? opt : null))}
                ...
                <div className="flex-[1] flex justify-center px-1 sm:px-2">
                  {isManagerView ? (
                    <input
                      ...
                      onChange={(e) =>
                        updateComponent(comp.id, 'unitLaborCost', parseFloat(e.target.value) || 0)
                      }
                ...
                {isManagerView && (
                  <div className="flex-[1] flex justify-center px-1 sm:px-2">
                    <input
                      ...
                      onChange={(e) =>
                        updateComponent(comp.id, 'laborHours', parseFloat(e.target.value) || 0)
                      }
                      className="w-12 sm:w-16 bg-gray-700/50 border border-gray-600 text-white rounded px-1 py-[2px] text-[10px] sm:text-xs focus:outline-none"
                    />
                  </div>
