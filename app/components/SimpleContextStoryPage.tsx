...
import type { PortfolioContext } from '@/lib/portfolioContent';

type ContextStoryStep = 'summary' | 'why' | 'proof' | 'next';

const orderedSteps: ContextStoryStep[] = ['summary', 'why', 'proof', 'next'];

function stepNumber(step: ContextStoryStep) {
  return orderedSteps.indexOf(step) + 1;
}

export default function SimpleContextStoryPage({ context }: { context: PortfolioContext }) {
  const [step, setStep] = useState<ContextStoryStep>('summary');
  const progress = stepNumber(step);
  const firstArtifact = context.featuredArtifacts[0];
  const secondArtifact = context.featuredArtifacts[1];
            ...
            {step === 'summary' ? (
              <div className="space-y-8">
                <div className="space-y-3">
                  ...
                  <p className="max-w-2xl text-lg leading-8 text-slate-700">{context.summary}</p>
                </div>
              </div>
            ) : null}
            ...
            {step === 'next' ? (
              <div className="space-y-8">
                <div className="space-y-3">
                  ...
                  <p className="max-w-2xl text-lg leading-8 text-slate-700">
                    You can open the main proof, see another proof item, or go back to the guided
                    start page.
                  </p>
                </div>
                <div className="grid gap-4">
                  ...
                  {secondArtifact ? (
                    <Link
                      href={secondArtifact.href}
                      className="inline-flex min-h-16 items-center justify-center rounded-[1.6rem] border-2 border-slate-300 bg-white px-6 py-4 text-xl font-semibold text-slate-950 transition hover:border-slate-500"
                    >
                      {secondArtifact.label}
                    </Link>
                  ) : null}
                  <Link
                    href="/"
