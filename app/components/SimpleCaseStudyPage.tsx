...
import type { CaseStudy } from '@/lib/showcaseContent';

type CaseStudyStep = 'summary' | 'challenge' | 'architecture' | 'outcomes' | 'next';

const orderedSteps: CaseStudyStep[] = ['summary', 'challenge', 'architecture', 'outcomes', 'next'];

function stepNumber(step: CaseStudyStep) {
  return orderedSteps.indexOf(step) + 1;
}

export default function SimpleCaseStudyPage({ caseStudy }: { caseStudy: CaseStudy }) {
  const [step, setStep] = useState<CaseStudyStep>('summary');
            ...
            {step === 'summary' ? (
              <div className="space-y-8">
                <div className="space-y-3">
                  ...
                  <p className="max-w-2xl text-lg leading-8 text-slate-700">{caseStudy.summary}</p>
                </div>
              </div>
            ) : null}
            ...
            {step === 'outcomes' ? (
              <div className="space-y-8">
                ...
                <div className="grid gap-4">
                  {caseStudy.outcomes.slice(0, 3).map((item) => (
                    <div
                      key={item}
